...
theme.color = {
	normal  = { bg = { 0.5, 0.5, 0.5 }, fg = { 0.1, 0.1, 0.1 } },
	hovered = { bg = { 0, 0.4, 0.75 }, fg = { 0.2, 0.2, 0.2 } },
	active  = { bg = { 0, 0, 0.5 }, fg = { 0.3, 0.3, 0.3 } }
}
...
function theme.drawBox(x, y, w, h, colors, cornerRadius, type)
	...
	love.graphics.rectangle(type or 'fill', x, y, w, h, cornerRadius)
end
...
function theme.Label(text, opt, x, y, w, h)
	...
	love.graphics.printf(text, floor(x + 2), floor(y), w - 4, opt.align or "center")
end
...
function theme.Checkbox(chk, opt, x, y, w, h)
	...
	if chk.text then
		love.graphics.setFont(opt.font)
		y = y + theme.getVerticalOffsetForAlign(opt.valign, opt.font, h)
		love.graphics.printf(chk.text, floor(x + h), floor(y), w - h, opt.align or "left")
	end
end
...
function theme.Input(input, opt, x, y, w, h)
	...
	love.graphics.setFont(opt.font)
	love.graphics.print(input.text, floor(x), floor(y + (h - th) / 2))

	-- candidate text
	local tw = opt.font:getWidth(input.text)
	local ctw = opt.font:getWidth(input.candidate_text.text)
	love.graphics.setColor((opt.color and opt.color.normal and opt.color.normal.fg) or theme.color.normal.fg)
	love.graphics.print(input.candidate_text.text, floor(x + tw), floor(y + (h - th) / 2))
