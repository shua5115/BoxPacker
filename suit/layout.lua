...
function Layout:push(x,y,w,h)
	...
	-- Modified, padx and pady unused
	-- return self:reset(x,y, padx or self._padx, pady or self._pady)
	self:reset(x or self._x ,y or self._y , self._padx, self._pady)
	self._w = w or self._w
	self._h = h or self._w
	return self
end
...
function Layout:pop()
	...
	self._w, self._h = self._w or w or 0, self._h or h or 0
